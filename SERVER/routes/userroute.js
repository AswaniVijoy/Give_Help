...
// POST /user/login — sets cookie (theme pattern) AND returns token (React pattern)
...
// POST /user/donate (protected, User only)
user.post("/donate", authenticate, async (req, res) => {
  try {
    ...
    if (!campaignData) return res.status(404).json({ msg: "Campaign not found" });
    ...
    if (updatedCampaign.Goal && updatedCampaign.Raised >= updatedCampaign.Goal) {
      return res.status(201).json({ msg: "Donation successful! Campaign goal fully achieved!", Raised: updatedCampaign.Raised });
    }

    res.status(201).json({ msg: "Donation successful", Raised: updatedCampaign.Raised, Remaining: updatedCampaign.Goal - updatedCampaign.Raised });
  } catch (err) {
    res.status(500).json({ msg: "Server error", error: err.message });
  }
});

// GET /user/my (protected) — user's own donations
