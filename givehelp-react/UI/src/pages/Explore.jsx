...
const Explore = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:2000/campaign")
      .then(res => res.json())
      .then(data => {
  ...
  // Filter + sort on frontend whenever category or sort changes
  useEffect(() => {
    ...
    // 2. Sort
    if (sort === "Most Funded") {
      // Highest Raised amount first
      result.sort((a, b) => (b.Raised || 0) - (a.Raised || 0));
    } else if (sort === "% Complete") {
      // Highest percentage toward goal first
      result.sort((a, b) => {
      ...
      // Default: Latest — newest CreatedAt first
  ...
  const sortLabel = () => {
    if (sort === "Most Funded") return "sorted by most raised";
    if (sort === "% Complete") return "sorted by % toward goal";
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {filtered.length} campaign{filtered.length !== 1 ? "s" : ""} — {sortLabel()}
            </p>
          </div>
                    ...
                    <div className="mt-4">
                      ...
                      <div className="mt-2 flex items-center justify-between text-sm text-gray-600">
                        <div>
                          ...
                          {c.Goal > 0 && (
                            <span className="text-gray-400"> / Rs.{c.Goal?.toLocaleString()}</span>
                          )}
                        </div>
                        <Link to={`/campaign/${c._id}`}>
                          <button className="bg-black text-white rounded-md px-4 py-2 hover:bg-gray-800 transition text-sm">
                            Donate
                          </button>
                        </Link>
                      </div>
