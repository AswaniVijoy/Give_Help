...
const Home = () => {
  ...
  useEffect(() => {
    ...
    // "All" uses /featured — top 6 most funded campaigns
    // Category filter uses /category/:cat then sorts by most funded on frontend
    const url = category === "All"
      ? "http://localhost:2000/campaign/featured"
      : `http://localhost:2000/campaign/category/${category}`;
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        // For category view: sort by most funded, take top 6
        const sorted = category === "All"
          ? data  // already sorted + limited by backend
          : data
  ...
  return (
    <div className="bg-gray-50 text-gray-800">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {category === "All"
                  ? "Top 6 most funded campaigns"
                  : "Sorted by most funded"}
              </p>
            </div>
                ...
                return (
                  <article key={c._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:scale-105 transition">
                    <div className="h-40 bg-gray-100 relative">
                      {c.Image ? (
                        <img
                          src={`http://localhost:2000/campaign/image/${encodeURIComponent(c.Title)}`}
                          alt={c.Title}
                      ...
                      <div className="mt-4">
                        ...
                        <div className="mt-2 flex items-center justify-between text-sm text-gray-600">
                          ...
                          <Link to={`/campaign/${c._id}`}>
                            <button className="bg-black rounded-md text-white px-4 py-2 hover:bg-gray-700 transition">
                              Donate
                            </button>
                          </Link>
                        </div>
      ...
      <footer className="border-t py-6 text-center text-sm text-gray-500">
        &copy; 2025 GiveHelp. All rights reserved.
      </footer>
    </div>
  );
};
